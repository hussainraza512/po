...
// Function to set the fill property based on the theme
const setFillProperty = (theme) => {
    const homeBlob = document.querySelector('.home-blob');
    if (theme === 'dark') {
        homeBlob.style.fill = 'var(--body-color)';
    } else {
        // Set the fill property to the default value for light mode
        homeBlob.style.fill = ''; // You can set it to another color if needed
    }
};
...
themeButton.addEventListener('click', () => {
    ...
    // Set the fill property based on the current theme
    setFillProperty(getCurrentTheme());
...
window.addEventListener('resize', function () {
    var closeElement = document.querySelector('.close1');
    var screenWidth = window.innerWidth;

    // Toggle the class based on the screen width
    if (screenWidth >= 768) {
        closeElement.classList.add('responsive-display');
    } else {
        closeElement.classList.remove('responsive-display');
    }
});